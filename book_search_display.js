    ...
    $.ajax({
      ...
      success:function(data){
        ...
        if (data.isJungDo == false){
          BookLocData = "상주캠퍼스";
          BookStateData = "대출가능";
          //console.log([BookLocData, BookStateData]);
          //console.log(data.isJungDo);
        }
        else{
          //console.log(data.list[0].location);
          BookLocData = data.list[0].location;
          BookStateData = data.list[0].state;
        }
      }
    });
}
  ...
  $.ajax({
    ...
    data:{type:type_arr[type],
      ...
      for(var i=0;i<ParsedData.list.length;i++){
        ...
        SearchBookLoc(ParsedData.list[i].id);
        var Arry = [BookLocData, BookStateData];
      ...
      '<img class="book-img" src="'+ temp_imagechecker +'" alt="'+ParsedData.list[i].title+'">'
        ...
        +'<div>'
          ...
          +'<span class="book-status"> '+Arry[0]+' / '+Arry[1]+' </span>'
